...
- Completed immersive refinement (supersedes the previous native-cursor implementation below): tilted -18° gold trishul rendered as a portal overlay, 430ms click recoil with a small eight-particle gold flourish, retained event-driven dot mesh, normal native affordances over editing/disabled/map/WebGL targets. Pointer is restored for touch, keyboard traversal, reduced motion, and blur. DOM refs are captured in effect for safe cleanup.
- Completed cultural motion: original alpona-inspired SVG, 75-second page/hero rotations and a 90-second floor-light pattern. Reduced-motion preferences stop decorative animation.
- Completed sanctuary detailing: procedural cloth/stone/tile textures; fluted lathed terracotta columns and mouldings; pleated vermilion drapes; brass arch bells; instanced marigold garlands; framed photo shrine and stepped altar; lathed brass lamps; thali/sweets, conch and ritual vessel; soft sprite incense and subtle dust; stylized dhoti-clad worshipper with an animated dhunuchi gesture. The site remains transparent that this is an artistic scene, not a scanned real venue.
- New sanctuary controls: “Watch the ritual” close-up and sanctuary camera views, manual pause/resume, live reduced-motion handling, focus/visibility/mostly-offscreen auto-pause, Escape to leave expanded view, larger mobile controls. Existing entry/exit, three discovery hotspots, lamps, flowers, reset and expand remain working; new visible content translated EN/BN.
- Final upgrade verification: clean production build; testing report `/app/test_reports/iteration_2.json` initially found motion-preference synchronization, background pausing, map readiness selector, small mobile labels. Fixed with `useSyncExternalStore` media hook, explicit focus/visibility/IntersectionObserver activity hook, a real map-container test marker and ResizeObserver, and 42px mobile control targets. Three repeated reduced/no-preference cycles, manual pause, focus pause/resume, mostly-offscreen pause/resume, and three sanctuary/map round trips passed subsequent browser checks. See `/app/test_reports/immersive_upgrade_final.md`.
...
## Latest user request / approved direction
User: “i guess the still trishul is not matching, what if its tilted and on clicking it shows some movement, your wish what you like to od. change it in yur way. also add some animated props, maybe some cultural designs alpona moving in circle, then you can also add some animated person giving puja. also you can upgrade the 3d pandal i guess, make a real world stuff out there, more details”

Approved: “Go ahead: tilted gold trishul with a small click flourish, slowly rotating alpona, a stylized animated worshipper, and a richer 3D pandal with detailed materials, offerings, incense, and lighting.” Keep movement respectful/subtle, preserve reduced-motion support, and identify the scene as an artistic reconstruction rather than a venue scan.